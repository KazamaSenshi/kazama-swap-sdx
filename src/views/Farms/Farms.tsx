...
import { Image, Heading, RowType, Toggle, Text, Button, ArrowForwardIcon, Flex, Card } from '@kazamaswap/uikit'
...
import KazamaFarmHeader from 'components/KazamaFarmHeader'
...
const getDisplayApr = (kazamaRewardsApr?: number, lpRewardsApr?: number) => {
...
const Farms: React.FC = () => {
    ...
    (farmsToDisplay: DeserializedFarm[]): FarmWithStakedValue[] => {
      let farmsToDisplayWithAPR: FarmWithStakedValue[] = farmsToDisplay.map((farm) => {
        ...
        const { kazamaRewardsApr, lpRewardsApr } = isActive
          ? getFarmApr(new BigNumber(farm.poolWeight), kazamaPrice, totalLiquidity, farm.lpAddresses[ChainId.MAINNET])
  ...
  const rowData = chosenFarmsMemoized.map((farm) => {
    ...
    const lpLabel = farm.lpSymbol && farm.lpSymbol.split(' ')[0].toUpperCase().replace('KAZAMA', '')
    ...
    <>
      ...
      <Flex flexDirection={['column-reverse', null, 'row']}>
       <Left>
        ...
        <KazamaText>
          {t('Stake Kazama-LP tokens and farm them')}
        </KazamaText>
        <KazamaText>
          {t('to start earning.')}
        </KazamaText>
