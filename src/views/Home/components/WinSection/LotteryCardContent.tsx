...
import { Flex, Text, Skeleton, Link, Button, ArrowForwardIcon } from '@kazamaswap/uikit'
...
const LotteryCardContent = () => {
  ...
  const kazamaPriceBusdAsString = usePriceKazamaBusd().toString()

  const kazamaPrizesText = t('%kazamaPrizeInUsd% in KAZAMA prizes this round', { kazamaPrizeInUsd: kazamaPriceBusdAsString })
  const [pretext, prizesThisRound] = kazamaPrizesText.split(kazamaPriceBusdAsString)

  const kazamaPriceBusd = useMemo(() => {
    return new BigNumber(kazamaPriceBusdAsString)
  }, [kazamaPriceBusdAsString])
  ...
  useEffect(() => {
    ...
    const fetchCurrentLotteryPrize = async () => {
      const { amountCollectedInKazama } = await fetchLottery(lotteryId)
      const prizeInBusd = kazamaPriceBusd.times(amountCollectedInKazama)
  ...
  }, [lotteryId, slowRefresh, setCurrentLotteryPrize, kazamaPriceBusd])
