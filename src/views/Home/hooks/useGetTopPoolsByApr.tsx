...
import { fetchKazamaVaultFees, fetchPoolsPublicDataAsync } from 'state/pools'
import { simpleRpcProvider } from 'utils/providers'
import { useKazamaVault, usePools } from 'state/pools/hooks'
...
const useGetTopPoolsByApr = (isIntersecting: boolean) => {
  ...
  const pools = useMemo(() => {
    ...
    const kazamaPool = activePools.find((pool) => pool.sousId === 0)
    const kazamaAutoVault = { ...kazamaPool, isAutoVault: true }
    const kazamaAutoVaultWithApr = { ...kazamaAutoVault, apr: getAprData(kazamaAutoVault, performanceFeeAsDecimal).apr }
    return [kazamaAutoVaultWithApr, ...poolsWithoutAutoVault]
  ...
  useEffect(() => {
    const fetchPoolsPublicData = async () => {
      ...
      try {
        await dispatch(fetchKazamaVaultFees())
  ...
  }, [setTopPools, pools, fetchStatus, kazamaPriceBusd, topPools, performanceFeeAsDecimal])
