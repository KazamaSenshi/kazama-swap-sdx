...
import { useCompetitionKazamaRewards, getRewardGroupAchievements } from '../../helpers'
...
const UserPrizeGrid: React.FC<{ userTradingInformation?: UserTradingInformationProps }> = ({
  ...
  const { kazamaReward, dollarValueOfKazamaReward } = useCompetitionKazamaRewards(userCakeRewards)
        ...
        <tr>
          <BoldTd>
            <Flex flexDirection="column">
              <Text bold>{kazamaReward.toFixed(2)}</Text>
              {dollarValueOfKazamaReward ? (
                <Text fontSize="12px" color="textSubtle">
                  ~{dollarValueOfKazamaReward} USD
                </Text>
              ) : (
