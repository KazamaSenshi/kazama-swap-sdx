...
import { Modal, Text, Flex, Button, HelpIcon, AutoRenewIcon, useTooltip } from '@kazamaswap/uikit'
import { getBalanceNumber } from 'utils/formatBalance'
import { useKazamaVaultContract } from 'hooks/useContract'
...
const BountyModal: React.FC<BountyModalProps> = ({ onDismiss, TooltipComponent }) => {
  ...
  const kazamaVaultContract = useKazamaVaultContract()
  ...
  const totalYieldToDisplay = getBalanceNumber(totalPendingKazamaHarvest, 18)
  ...
  const handleConfirmClick = async () => {
    setPendingTx(true)
    try {
      const tx = await callWithGasPrice(kazamaVaultContract, 'harvest', undefined, { gasLimit: 300000 })
      const receipt = await tx.wait()
      if (receipt.status) {
        toastSuccess(
          t('Bounty collected!'),
          <ToastDescriptionWithTx txHash={receipt.transactionHash}>
            {t('KAZAMA bounty has been sent to your wallet.')}
    ...
    <Modal title={t('Claim Bounty')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <Flex alignItems="flex-start" justifyContent="space-between">
        ...
        <Flex flexDirection="column">
          <Balance bold value={kazamaBountyToDisplay} decimals={2} unit=" KAZAMA" />
      ...
      <Flex alignItems="center" justifyContent="space-between">
        ...
        <Balance color="textSubtle" value={totalYieldToDisplay} unit=" KAZAMA" />
      </Flex>
      ...
      {account ? (
        <Button
          isLoading={pendingTx}
          disabled={!dollarBountyToDisplay || !kazamaBountyToDisplay || !callFee}
          endIcon={pendingTx ? <AutoRenewIcon spin color="currentColor" /> : null}
          onClick={handleConfirmClick}
          mb="28px"
          id="autoKazamaConfirmBounty"
        >
