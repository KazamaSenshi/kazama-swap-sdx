...
import { getAddress, getKazamaVaultAddress } from 'utils/addressHelpers'
import { BIG_ZERO } from 'utils/bigNumber'
import { registerToken } from 'utils/wallet'
import { getBalanceNumber, getFullDisplayBalance } from 'utils/formatBalance'
import { convertSharesToKazama, getPoolBlockInfo } from 'views/Pools/helpers'
...
const ActionPanel: React.FC<ActionPanelProps> = ({ account, pool, userDataLoaded, expanded, breakpoints }) => {
  ...
  const kazamaVaultContractAddress = getKazamaVaultAddress()
  ...
  const getTotalStakedBalance = () => {
    ...
    if (isManualKazamaPool) {
      const manualKazamaTotalMinusAutoVault = new BigNumber(totalStaked).minus(totalKazamaInVault)
    ...
    <StyledActionPanel expanded={expanded}>
      <InfoSection>
        ...
        {poolContractAddress && (
          <Flex mb="8px" justifyContent={['flex-end', 'flex-end', 'flex-start']}>
            <LinkExternal
              href={`${BASE_BSC_SCAN_URL}/address/${isAutoVault ? kazamaVaultContractAddress : poolContractAddress}`}
              bold={false}
            >
              {t('View Contract')}
            </LinkExternal>
      ...
      <ActionContainer>
        {showSubtitle && (
          <Text mt="4px" mb="16px" color="textSubtle">
            {isAutoVault ? t('Automatic restaking') : `${t('Stake')} KAZAMA, ${t('earn').toLocaleLowerCase()} KAZAMA`}
          </Text>
        )}
        {pool.isAutoVault ? (
