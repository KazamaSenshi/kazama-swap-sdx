...
import { useSenshiMaster, useSousChef } from 'hooks/useContract'
...
const useUnstakePool = (sousId: number, enableEmergencyWithdraw = false) => {
  ...
  const handleUnstake = useCallback(
    async (amount: string, decimals: number) => {
      if (sousId === 0) {
        await unstakeFarm(SenshiMasterContract, 0, amount)
    ...
    [account, dispatch, enableEmergencyWithdraw, SenshiMasterContract, sousChefContract, sousId],
  )
