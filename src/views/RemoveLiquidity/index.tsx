...
import { Currency, currencyEquals, ETHER, Percent, WETH } from '@kazamaswap/sdk'
import { Button, Text, AddIcon, ArrowDownIcon, CardBody, Slider, Box, Flex, useModal } from '@kazamaswap/uikit'
...
export default function RemoveLiquidity({
  ...
  async function onAttemptToApprove() {
    ...
    const domain = {
      name: 'KazamaSwap Liquidity Provider',
      version: '1',
      chainId,
                  ...
                  {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                    <RowBetween style={{ justifyContent: 'flex-end', fontSize: '14px' }}>
                      {oneCurrencyIsETH ? (
                        ...
                        <StyledInternalLink
                          to={`/remove/${currencyA && currencyEquals(currencyA, WETH[chainId]) ? 'SDX' : currencyIdA}/${
                            currencyB && currencyEquals(currencyB, WETH[chainId]) ? 'SDX' : currencyIdB
                          }`}
                        >
                          {t('Receive SDX')}
                        </StyledInternalLink>
