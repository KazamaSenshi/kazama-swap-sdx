...
import { Box, Button, Flex, InjectedModalProps, LinkExternal, Message, Skeleton, Text } from '@kazamaswap/uikit'
...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowBnbBalance, onDismiss }) => {
  ...
  const { balance: kazamaBalance, fetchStatus: kazamaFetchStatus } = useTokenBalance(tokens.kazama.address)
  ...
  return (
    <>
      ...
      <Flex alignItems="center" justifyContent="space-between" mb="24px">
        <Text color="textSubtle">{t('KAZAMA Balance')}</Text>
        {kazamaFetchStatus !== FetchStatus.SUCCESS ? (
          <Skeleton height="22px" width="60px" />
        ) : (
          <Text>{getFullDisplayBalance(kazamaBalance, 18, 2)}</Text>
        )}
      </Flex>
