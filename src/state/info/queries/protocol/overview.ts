...
const formatKazamaFactoryResponse = (rawKazamaFactory?: KazamaFactory) => {
  if (rawKazamaFactory) {
    return {
      totalTransactions: parseFloat(rawKazamaFactory.totalTransactions),
      totalVolumeUSD: parseFloat(rawKazamaFactory.totalVolumeUSD),
      totalLiquidityUSD: parseFloat(rawKazamaFactory.totalLiquidityUSD),
    }
  }
  return null
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatKazamaFactoryResponse(data?.kazamaFactories?.[0])
      const overviewData24 = formatKazamaFactoryResponse(data24?.kazamaFactories?.[0])
      const overviewData48 = formatKazamaFactoryResponse(data48?.kazamaFactories?.[0])
