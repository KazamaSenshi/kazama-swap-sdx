...
import { KazamaDayDatasResponse } from '../types'
import { fetchChartData, mapDayData } from '../helpers'

/**
 * Data for displaying Liquidity and Volume charts on Overview page
 */
const KAZAMA_DAY_DATAS = gql`
  query overviewCharts($startTime: Int!, $skip: Int!) {
    kazamaDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      date
      dailyVolumeUSD
      totalLiquidityUSD
    }
...
const getOverviewChartData = async (skip: number): Promise<{ data?: ChartEntry[]; error: boolean }> => {
  try {
    const { kazamaDayDatas } = await request<KazamaDayDatasResponse>(INFO_CLIENT, KAZAMA_DAY_DATAS, {
      startTime: PCS_V2_START,
      skip,
    })
    const data = kazamaDayDatas.map(mapDayData)
