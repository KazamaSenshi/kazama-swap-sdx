...
const lotteryAddress = getKazamaLotteryAddress()
...
const fetchKazamaRewardsForTickets = async (
  ...
  try {
    const kazamaRewards = await multicallv2(kazamaLotteryAbi, calls)

    const kazamaTotal = kazamaRewards.reduce((accum: BigNumber, kazamaReward: ethers.BigNumber[]) => {
